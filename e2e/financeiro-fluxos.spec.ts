...
test.describe("Fluxos financeiros", () => {
  ...
  test("cria receita, cria despesa, edita e exclui", async ({ page }) => {
    ...
    await expect(page.getByRole("row").filter({ hasText: receitaDescricao }).getByText(/R\$\s*1\.000,00/)).toBeVisible();
    ...
    await expect(page.getByRole("row").filter({ hasText: despesaDescricao }).getByText(/R\$\s*660,00/)).toBeVisible();

    await editFirstEntry(page, receitaDescricao, receitaEditada, "1660");
    await expect(page.getByText(receitaEditada)).toBeVisible();
    await expect(page.getByRole("row").filter({ hasText: receitaEditada }).getByText(/R\$\s*1\.660,00/)).toBeVisible();
