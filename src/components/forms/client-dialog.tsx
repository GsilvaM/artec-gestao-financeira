    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) { setForm(initialForm); setErrors({}); } onOpenChange(v); }}>
      <DialogContent className="relative">
        ...
        <DialogHeader>
          ...
          <DialogDescription>{isEditing ? "Altere os dados do registro." : "Cadastre dados basicos para vincular contas e lancamentos."}</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Nome" error={errors.nome}><Input value={form.nome} onChange={(e) => updateField("nome", e.target.value)} placeholder="Nome ou razao social" /></Field>
          ...
          <div className="sm:col-span-2"><Field label="Endereco/observacoes"><Textarea value={form.observacoes} onChange={(e) => updateField("observacoes", e.target.value)} placeholder="Endereco, bairro, cidade ou observacoes" /></Field></div>
        </div>
        <DialogFooter>
