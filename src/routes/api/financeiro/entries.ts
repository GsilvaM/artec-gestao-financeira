...
const createSchema = z.object({ ...financialEntryCreateSchema.shape, userId: uuidField });
...
export async function action({ request, params }: RouteArgs) {
  ...
  try {
    if (request.method === "POST") {
      ...
      return json(await financialEntryRepo.create(createSchema.parse(body)), { status: 201 });
    }
